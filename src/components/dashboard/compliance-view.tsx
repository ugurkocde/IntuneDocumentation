      ...
      {capability.checks.map((check, index) => (
        ...
        >
          <div className="flex flex-wrap items-center gap-2">
            ...
            {check.result && (
              <span
                className={`rounded-full px-2 py-1 font-semibold ${check.result === "matches" ? "bg-emerald-50 text-emerald-800" : check.result === "different" ? "bg-red-50 text-red-800" : "bg-amber-50 text-amber-800"}`}
              >
          ...
          {check.policyName && (
            <p className="mt-2 font-semibold">{check.policyName}</p>
          )}
          ...
          <dl className="mt-2 grid gap-2 sm:grid-cols-2">
            <div>
              ...
              <dd className="mt-1 break-words">
                {displayCheckValue(check.expectedValue) ||
                  "No automated comparison available"}
              </dd>
            </div>
            <div>
          ...
          {check.reason && (
            <p className="mt-2 text-slate-600">{check.reason}</p>
          )}
          {check.assignment && (
              ...
              <section key={capabilityId} className="py-4">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <span className="text-xs text-slate-600">
                    {checkSummary(capability.checks)}
                  </span>
                </div>
...
export function ComplianceView({
  ...
  const selectedChecks = [
    ...new Set(
      selectedFramework?.controls.flatMap((control) => control.capabilityIds) ??
        [],
    ),
  ]
              ...
              <AnimatePresence mode="wait" initial={false}>
                ...
                >
                  ...
                  <p className="text-xs text-slate-600">
                    {
                      selectedChecks.filter(
                        (check) => check.assessmentStatus === "checked",
                      ).length
                    }{" "}
                    checked;{" "}
                    {
